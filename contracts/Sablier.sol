...
import "./compound/CarefulMath.sol";
import "./Types.sol";
 ...
 * @author Sablier
 */
abstract contract Sablier is IERC1620, Ownable, CarefulMath, ReentrancyGuard {
    /*** Storage Properties ***/
    address caller;
    address bondingShares;
    uint dustShares;
    //TODO: add a method to collect dustShares
    ...
    */
    ...
    constructor(address _caller, address _bondingShares) public { //what address(s) should go here if any?
        //require(cTokenManagerAddress != address(0x00), "cTokenManager contract is the zero address");
        //Ownable.initialize(msg.sender);
        caller = _caller;
        bondingShares = _bondingShares;
        nextStreamId = 1;
    }
...
*/
    ...
    */
    ...
    function setBondingShares(address newBondingShares) public onlyOwner {
      bondingShares = newBondingShares;
    }
    ...
    */
        ...
        returns (uint256)
    {
        ...
        if(deposit % vars.duration != 0) {
          uint mod = deposit % vars.duration;
          (vars.mathErr, deposit) = subUInt(deposit, mod);
          assert(vars.mathErr == MathError.NO_ERROR);
          if(tokenAddress == bondingShares) {
            dustShares += mod;
          }
        }
